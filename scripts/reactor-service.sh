  ...
  if [[ -z "${GITHUB_TOKEN:-}" ]]; then
    if [[ -f "/home/ray/.config/gh/hosts.yml" ]]; then
      ...
      if [[ -n "$GITHUB_TOKEN" ]]; then
        echo "OpenReactor reactor is falling back to the GitHub CLI token. Add GitHub App credentials to $ENV_FILE to switch to app auth." >&2
      fi
    fi
    if [[ -z "${GITHUB_TOKEN:-}" ]]; then
    ...
    if [[ -z "${GITHUB_TOKEN:-}" ]]; then
      echo "OpenReactor reactor is missing GitHub auth." >&2
