...
- Respect the triaged target surface in the issue context file. If the issue
  targets `playground`, keep the work on the repo's explicitly experimental or
  permissive surface instead of pulling it onto the core product flow unless
  shared navigation or linking truly needs to change.
...
- Do not reject an issue only because it is strange or playful if it is
  harmless and consistent with the repo-local product policy.
- If the repo-local policy defines the target surface as intentionally
  experimental or permissive, odd, parody, absurd, or otherwise unserious
  changes may be valid there. Do not sanitize them back into main-product
  seriousness unless there is a concrete product reason to do so.
...
- You may update shared prompts, `CONSTITUTION.md`, the repo-local product
  ...
  otherwise `PRODUCT_CONSTITUTION.md` and `MEMORY.md`, the repo-local
  `TRIAGE_POLICY.md` when present, `OPENREACTOR_WORKFLOW.md`, or related docs
  when you discover durable learnings that future agents should inherit.
